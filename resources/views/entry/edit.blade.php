...
<x-app-layout>
            ...
            @foreach ($journalEntry->files as $file)
                <div class="relative group">
                    @if (strpos($file->mime, 'video') !== false)
                        ...
                        cursor-pointer" onclick="showVideo(event, '{{ $file->path }}', '{{ $file->mime }}')">
        ...
        <h2 class="text-xl text-gray-700 mt-8 mb-4">Edit Entry</h2>
            <form action="{{ route('entries.update', $journalEntry->id) }}" method="POST" class="w-full" enctype="multipart/form-data">
                ...
                <label class="form-control">
                    ...
                    <textarea class="w-full rounded-sm mb-4 mt-2 textarea textarea-bordered h-48"
                    name="description" rows=5 required>{{$journalEntry->description}}</textarea>
                </label>
    ...
    <form id="delete-entry-form" class="hidden" action="{{
    route('entries.destroy', $journalEntry->id) }}" method="POST">
        @csrf
        @method('DELETE')
    </form>
