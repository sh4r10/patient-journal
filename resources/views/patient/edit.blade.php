...
<x-app-layout>
    ...
    <div onclick="closeDeleteConfirmation(event)" id="delete_confirmation"
        ...
        <div class="bg-white max-w-screen-sm w-full p-8 rounded" onclick="showDeleteConfirmation(event)">
            ...
            <form action="{{ route('patients.destroy', $patient) }}" method="post" id="delete-form">
                @csrf
                @method('DELETE')
                <!-- Password -->
                <div class="mt-4">
                ...
                <form action="{{ route('patients.update', $patient) }}" method="POST" class="w-full">
                    ...
                    <div class="flex gap-6 mt-4 mb-8 w-full">
                        <label class="w-full">
                            ...
                            <input type="text" class="rounded w-full" name="name" value={{ $patient->name }}
                                class="mt-2 focus:border-slate-500 focus:ring-0 w-full" required />
                        </label>
                        <label class="w-full">
                            <div class="label text-slate-600 mb-2 text-sm">
                    ...
                    <div class="flex gap-6 mb-8">
                        <label class="w-full">
                            ...
                            <input type="email" class="rounded w-full" name="email" value={{ $patient->email }}
                                class="mt-2 focus:border-slate-500 focus:ring-0 w-full" required />
                        </label>
                        <label class="w-full">
                            <div class="label text-slate-600 mb-2 text-sm">
                            ...
                            <input type="text" class="rounded w-full" name="phone" value={{ $patient->phone }}
                                class="mt-2 focus:border-slate-500 focus:ring-0 w-full" required />
                        </label>
                    </div>
                    <div
