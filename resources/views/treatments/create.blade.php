...
<x-app-layout>
    <div class="mt-16 max-w-screen-md m-auto">
        ...
        <form action="{{ route('treatments.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-slate-600 text-sm mb-2">Name:</label>
            ...
            <div class="mb-4">
                <label for="description" class="block text-slate-600 text-sm mb-2">Description:</label>
                <textarea name="description" rows=8 id="description"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-slate-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description') }}</textarea>
                @error('description')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="w-full flex flex-row-reverse justify-start items-center gap-4 mt-4">
                ...
                text-blue-950 rounded-sm py-2 px-8"
                    href="{{ route('treatments.index') }}">Cancel</a>
            </div>
        </form>
    </div>
</x-app-layout>
